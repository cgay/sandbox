...
(custom-set-variables
 ...
 '(lsp-dylan-exe-pathname "/Users/cgay/dylan/opendylan/bin/dylan-lsp-server")
 '(lsp-dylan-extra-command-line-options '("--debug-server" "--debug-opendylan"))
 '(lsp-dylan-extra-server-flags '("--debug-opendylan"))
 '(lsp-dylan-open-dylan-registry "/Users/cgay/dylan/workspaces/opendylan/sources/registry")
 '(lsp-server-trace "messages")
 '(package-selected-packages
   '(ace-window racket-mode lsp-treemacs flycheck lsp-ui typescript-mode slime markdown-mode marginalia eglot yaml-mode hover lsp-mode protobuf-mode go-mode magit))
...
(setq display-buffer-alist
      ...
      '(
        ...
        ("\\*.+\\*"
         ...
         (mode . (compilation-mode dired-mode ibuffer-mode shell-mode))
...
;; DIME and LSP interfere with one another so we must choose.
(defvar using-dime-p t)
(if using-dime-p
     (progn
       (require 'dime)
       (dime-setup '(dime-repl dime-note-tree))
       (let* ((ws (concat *dylan* "/workspaces"))
              (project "deft"))         ; <<== Change this to what you're working on.
         (setq dime-dylan-implementations
               `((opendylan (,(if (equal project "dswank")
                                  (concat ws "/dylan-emacs-support/_build/bin/dswank")
                                (concat *dylan* "/opendylan/bin/dswank"))
                             "--debug"
                             ;; Use different port for dswank so I can debug dswank (in a
                             ;; different emacs) at the same time I'm using the main dswank.
                             "--port" ,(if (equal project "dswank")
                                           "4006"
                                         "4005"))
                            ;; https://github.com/dylan-lang/dylan-emacs-support/issues/76
                            :env (,(format "OPEN_DYLAN_USER_REGISTRIES=%s/%s/registry:%s/opendylan/sources/registry" ws project ws)
                                  ,(format "OPEN_DYLAN_USER_ROOT=%s/%s/_build" ws project))
                            ;; This sorta works, but setting USER_ROOT works better. Investigate.
   ...
   (progn
     ...
     ;; Let's not use lsp-mode automatically yet, while developing both DIME and LSP.
